...
exports.getSingleCompanyFinancialStatus = void 0;
const opendartFinanceService_1 = require("../services/opendartFinanceService");
const VALID_REPRT_CODES = new Set(["11011", "11012", "11013", "11014"]);
function defaultBusinessYear() {
    return String(new Date().getFullYear() - 1);
}
const getSingleCompanyFinancialStatus = async (req, res) => {
    try {
        ...
        if (!VALID_REPRT_CODES.has(reprtCode)) {
            return res.status(400).json({
                success: false,
                data: null,
                message: "reprtCode는 11011(사업), 11012(반기), 11013(1분기), 11014(3분기) 중 하나여야 합니다.",
            });
        }
        const data = await (0, opendartFinanceService_1.fetchSingleCompanyFinancialStatus)({
            corpCode,
            bsnsYear,
            reprtCode,
            fsDiv,
        ...
        return res.status(500).json({
            success: false,
            data: null,
            message: error instanceof Error ? error.message : "단일 기업 재무 상태 조회 중 오류가 발생했습니다.",
        });
    }
};
exports.getSingleCompanyFinancialStatus = getSingleCompanyFinancialStatus;
//# sourceMappingURL=financeControllers.js.map
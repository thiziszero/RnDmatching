...
exports.fetchSingleCompanyFinancialStatus = fetchSingleCompanyFinancialStatus;
const OPENDART_BASE_URL = process.env.OPENDART_BASE_URL ??
...
function findAmountByPriority(items, accountIdCandidates, accountNameCandidates) {
...
function buildFinancialMetrics(items) {
    const assets = findAmountByPriority(items, ["ifrs-full_Assets"], ["자산총계"]);
    const liabilities = findAmountByPriority(items, ["ifrs-full_Liabilities"], ["부채총계"]);
    const equity = findAmountByPriority(items, ["ifrs-full_Equity"], ["자본총계"]);
    const revenue = findAmountByPriority(items, ["ifrs-full_Revenue"], ["매출액", "수익(매출액)", "영업수익"]);
    const operatingIncome = findAmountByPriority(items, ["dart_OperatingIncomeLoss", "ifrs-full_OperatingProfitLoss"], ["영업이익", "영업손익"]);
    const netIncome = findAmountByPriority(items, ["ifrs-full_ProfitLoss"], ["당기순이익", "당기순손익"]);
    const currentAssets = findAmountByPriority(items, ["ifrs-full_CurrentAssets"], ["유동자산"]);
    const currentLiabilities = findAmountByPriority(items, ["ifrs-full_CurrentLiabilities"], ["유동부채"]);
    return {
        assets,
        liabilities,
        equity,
...
async function fetchSingleCompanyFinancialStatus(input) {
...
//# sourceMappingURL=opendartFinanceService.js.map
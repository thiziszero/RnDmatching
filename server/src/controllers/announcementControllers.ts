...
type RawFile = { fileName?: string, fileUrl?: string };
type RawItem = {
    subject?: string;
    viewUrl?: string;
    deptName?: string;
    ...
    managerTel?: string;
    pressDt?: string;
    files?: { file?: RawFile | RawFile[] } | Array<{ file?: RawFile | RawFile[] }>;
}
...
export const getList = async (req: Request, res: Response) => {
    try {
        ...
        const header = Array.isArray(responseData)
            ? responseData.find((entry: any) => entry?.header)?.header
            : responseData?.header;
        const body = Array.isArray(responseData)
            ? responseData.find((entry: any) => entry?.body)?.body
            : responseData?.body;
       ...
       if (header.resultCode != "00"){
        ...
        const rawItems = toArray<any>(body?.items?.item ?? body?.items);
        const items: RawItem[] = rawItems.map((entry: any) => {
        const item: RawItem = entry?.item ?? entry;
        const fileContainers = toArray<{ file?: RawFile | RawFile[] }>(item.files as { file?: RawFile | RawFile[] } | Array<{ file?: RawFile | RawFile[] }>);
        const files = fileContainers.flatMap((fileContainer) => toArray<RawFile>(fileContainer?.file));
